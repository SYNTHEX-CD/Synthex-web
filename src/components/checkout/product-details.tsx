  ...
  return (
    <div className="flex flex-col gap-3 px-3">
      <div className="flex items-start justify-between gap-4">
        ...
        {numberOfPrices > 1 && (
          <p className="text-muted-foreground pt-1 text-sm whitespace-nowrap">+{numberOfPrices - 1} de plus</p>
        )}
      </div>

      <div>
        <div className="flex flex-col">
          <p>{formatCurrency(checkoutData.totals.total, currency)} maintenant</p>
          {checkoutData.recurring_totals && billingCycle && (
            <p className="text-muted-foreground mt-1 text-sm">
              Puis {formatCurrency(checkoutData.recurring_totals.total, currency)}/{formatBillingCycle(billingCycle)}
              {trialPeriod && (
                <>
                  <span> après </span>
